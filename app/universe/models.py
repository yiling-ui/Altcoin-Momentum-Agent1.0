...
class UniverseConfig(_Mutable):
    ...
    # Construct-time throttle for ``UNIVERSE_FILTERED`` events. Defaults
    # to ``True`` so Phase 5's boot drill, replay, and reflection paths
    # observe every decision. Issue #6's full Top-200 scanner will flip
    # this to ``False`` to avoid bloating events.db at scan rate; a
    # per-call ``emit_event=True`` override on
    # :meth:`UniverseFilter.evaluate` still lets monitoring write an
    # on-demand audit-trail entry. Mirrors Phase 4's
