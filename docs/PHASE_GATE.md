...
| 11C.1C-C-B-B-B-D-B.1 | Historical Price Path Completeness / Kline Path Adapter v0 (*历史价格路径完整性 / K线路径适配器 v0*; paper / report / evidence-only first child slice under Phase 11C.1C-C-B-B-B-D-B) | 2026-05-27 | PR #71 implementation merged into `main` (new price path adapter `app/adaptive/post_discovery_price_path_adapter.py`; D-B / B1.1 evidence runner can load `data/historical_market_store/` daily top mover summary; price path resolution is now record-level, not symbol-only and no longer first-record-wins; operator-supplied price paths cannot turn a future price into the discovery anchor; Lookahead Guard hard-enforced at record level: `first_seen_time` only from a D-A audited record / `EventRepository` event timestamp, `first_seen_price` only from the latest price point at-or-before `first_seen_time`, post-first-seen path strictly after `first_seen_time` and only used for post-window outcome audit, peak / trough / MFE / MAE only computed after the window closes; v0 path is daily-bucket only, `kline_interval_used=1d`, NOT 1m / 5m intraday); the real main-branch B1.1 evidence was rerun on the operator VPS; this docs-only closeout PR records the resulting B1.1 main evidence run and flips the slice to **`ACCEPTED_TOOLCHAIN / PARTIAL_DATA_COVERAGE / DAILY_BUCKET_ONLY`** (explicitly **NOT** full data coverage accepted, explicitly **NOT** intraday-kline accepted). **B1.1 main evidence run output** (`data/reports/post_discovery_outcome/pr71_main_price_path_evidence`): `status=EVIDENCE_GENERATED`, `evaluated_count=300`, `report_generated_count=1`, event counts `POST_DISCOVERY_OUTCOME_EVALUATED=300` and `POST_DISCOVERY_OUTCOME_REPORT_GENERATED=1`, price path coverage `price_path_records_loaded=17` and `price_path_records_missing=283`, source summary `absent=283` and `historical_market_store_daily_top_movers=17`, missing-reason summary `no_first_seen_time=110`, `no_top_mover_row_covering_first_seen_time=133`, `insufficient_post_first_seen_points=40`, `kline_interval_used=1d`, marker `B1_1_PRICE_PATH_MAIN_EVIDENCE_CHECK=PASS`. **Notable symbols (still unresolved by B1.1 alone)**: `RAVEUSDT` (`loaded=false`, `loaded_record_count=0`, `missing_reason=no_top_mover_row_covering_first_seen_time`, `record_count=17`, `source=absent`); `STOUSDT` (`loaded=false`, `loaded_record_count=0`, `missing_reason=no_top_mover_row_covering_first_seen_time`, `record_count=3`, `source=absent`). **Warnings**: `d_a_backfill_records_missing_using_record_audited_fallback` (Format B fallback engaged; expected for the real D-A export shape). **What this acceptance level means**: B1.1 toolchain is accepted; the PR #71 evidence runner can evaluate 300 records and emit 300 `POST_DISCOVERY_OUTCOME_EVALUATED` events; the price path adapter works at daily-bucket resolution; the current local Historical Market Store only provided price paths for 17/300 records; 283/300 records remain missing price path; `RAVEUSDT` and `STOUSDT` remain unresolved because no top mover row covers their `first_seen_time`. **What this acceptance level does NOT mean**: B1.1 does **NOT** solve intraday price path completeness (the adapter is daily-bucket only; 283/300 records remain missing under the local store); B1.1 does **NOT** solve direction (no `long` / `short` / `entry` / `exit` / `stop` / `target` / `position_size` / `leverage` field is emitted; the labels and the price path are descriptive only); B1.1 does **NOT** prove strategy profitability (no PnL was simulated, no order was submitted, no Risk Engine decision was reproduced); B1.1 does **NOT** authorise auto-tuning (no `symbol_limit` expansion, anomaly threshold change, candidate-pool capacity change, Regime weight change, or any other runtime knob — "looking at the answer key" against the post-hoc D-A reference set or the daily-bucket price path is forbidden); B1.1 does **NOT** authorise DeepSeek trade decisions (DeepSeek remains read-only / sandbox-only / offline under the AI Layer Constitution); Phase 12 remains **FORBIDDEN**. **Next allowed route**: B1.1 closeout accepted as toolchain + partial data coverage + daily-bucket only — **B1.1 closes the B1 *Post-Discovery Outcome Metrics* path as `ACCEPTED_TOOLCHAIN / PARTIAL_DATA_COVERAGE / DAILY_BUCKET_ONLY`**; **next allowed route is B2 *Severe Missed Tail Triage v0*** (B2 will consume the `MISSED_STRONG_TAIL` / severe-miss records — including `RAVEUSDT` / `STOUSDT` — recorded by B1 / B1.1 and triage them as a structured slice; B2 must record, per severe miss, whether the case is blocked by missing intraday price path or by some other factor); **Historical Kline Store Builder / Intraday Price Path Backfill is NOT started now** and remains an optional future data-quality task only if B2 triage proves severe-miss attribution is blocked by missing intraday price paths, and only with explicit owner approval — it is **not** a precondition for B2 and is **not** the next slice. **B1.1 does not solve intraday price path completeness, does not solve direction, does not prove strategy profitability, and does not authorise auto-tuning.** **The next allowed route is NOT** to start DeepSeek directly, is **NOT** to start blind walk-forward directly, and is **NOT** to enter Phase 12. **Safety boundary held end-to-end**: `mode=paper`, `live_trading=False`, `exchange_live_orders=False`, `right_tail=False`, `llm=False`, `telegram_outbound_enabled=False`, `binance_private_api_enabled=False`, no private API, no live orders, no real Telegram outbound, no DeepSeek trade decision, **Phase 12 = FORBIDDEN**. The Risk Engine remains the single trade-decision gate. This closeout PR is **docs-only**: no file under `app/`, `scripts/`, `tests/`, `configs/`, `risk/`, `execution/`, `exchanges/`, `llm/`, `telegram/` is touched; no event name is changed; no schema version is changed; no test was run; no paper run, export, replay, or historical builder was invoked; no real API was contacted. See "Closed phase: Phase 11C.1C-C-B-B-B-D-B.1 (ACCEPTED_TOOLCHAIN / PARTIAL_DATA_COVERAGE / DAILY_BUCKET_ONLY)" block below. |
...
| 11C.1C-C-B-B-B-D-B.1 | Historical Price Path Completeness / Kline Path Adapter v0 (*历史价格路径完整性 / K线路径适配器 v0*; paper / report / evidence-only first child slice under Phase 11C.1C-C-B-B-B-D-B; ACCEPTED_TOOLCHAIN / PARTIAL_DATA_COVERAGE / DAILY_BUCKET_ONLY via PR #71 implementation + this docs-only closeout PR) | **ACCEPTED_TOOLCHAIN / PARTIAL_DATA_COVERAGE / DAILY_BUCKET_ONLY — see Closed phases table above** | Phase 11C.1C-C-B-B-B-D-B.1 is now **`ACCEPTED_TOOLCHAIN / PARTIAL_DATA_COVERAGE / DAILY_BUCKET_ONLY`** (PR #71 implementation merged into `main`; the real main-branch B1.1 evidence was rerun on the operator VPS; this docs-only closeout PR records the resulting B1.1 main evidence run). The full closeout — including the B1.1 main evidence run output (`status=EVIDENCE_GENERATED`, `evaluated_count=300`, `report_generated_count=1`, output dir `data/reports/post_discovery_outcome/pr71_main_price_path_evidence`), the event counts (`POST_DISCOVERY_OUTCOME_EVALUATED=300`, `POST_DISCOVERY_OUTCOME_REPORT_GENERATED=1`), the price path coverage (`price_path_records_loaded=17`, `price_path_records_missing=283`), the price path source summary (`absent=283`, `historical_market_store_daily_top_movers=17`), the price path missing-reason summary (`no_first_seen_time=110`, `no_top_mover_row_covering_first_seen_time=133`, `insufficient_post_first_seen_points=40`), the `kline_interval_used=1d` diagnostic, the notable-symbols block (`RAVEUSDT` and `STOUSDT` both `loaded=false`, `loaded_record_count=0`, `missing_reason=no_top_mover_row_covering_first_seen_time`, `record_count=17` and `record_count=3` respectively, `source=absent`), the `d_a_backfill_records_missing_using_record_audited_fallback` warning, the marker `B1_1_PRICE_PATH_MAIN_EVIDENCE_CHECK=PASS`, and the safety-flag invariants — is recorded under "Closed phase: Phase 11C.1C-C-B-B-B-D-B.1 (ACCEPTED_TOOLCHAIN / PARTIAL_DATA_COVERAGE / DAILY_BUCKET_ONLY)" below. **`ACCEPTED_TOOLCHAIN / PARTIAL_DATA_COVERAGE / DAILY_BUCKET_ONLY` is explicitly NOT full data coverage accepted and explicitly NOT intraday-kline accepted.** It records that B1.1 toolchain is accepted, that the PR #71 evidence runner can evaluate 300 records and emit 300 `POST_DISCOVERY_OUTCOME_EVALUATED` events, that the price path adapter works at daily-bucket resolution, that the current local Historical Market Store only provided price paths for 17/300 records, that 283/300 records remain missing price path, and that `RAVEUSDT` and `STOUSDT` remain unresolved because no top mover row covers their `first_seen_time`. **B1.1 does NOT solve intraday price path completeness. B1.1 does NOT solve direction. B1.1 does NOT prove strategy profitability. B1.1 does NOT authorise auto-tuning. B1.1 does NOT authorise DeepSeek trade decisions. Phase 12 remains FORBIDDEN.** Next allowed route: B1.1 closeout accepted as toolchain + partial data coverage + daily-bucket only — **B1.1 closes the B1 *Post-Discovery Outcome Metrics* path as `ACCEPTED_TOOLCHAIN / PARTIAL_DATA_COVERAGE / DAILY_BUCKET_ONLY`**; **next allowed route is B2 *Severe Missed Tail Triage v0*** (B2 will consume the `MISSED_STRONG_TAIL` / severe-miss records — including `RAVEUSDT` / `STOUSDT` — recorded by B1 / B1.1 and triage them as a structured slice; B2 must record, per severe miss, whether the case is blocked by missing intraday price path or by some other factor); **Historical Kline Store Builder / Intraday Price Path Backfill is NOT started now** and remains an optional future data-quality task only if B2 triage proves severe-miss attribution is blocked by missing intraday price paths, and only with explicit owner approval — it is **not** a precondition for B2 and is **not** the next slice. **B1.1 does not solve intraday price path completeness, does not solve direction, does not prove strategy profitability, and does not authorise auto-tuning.** The next allowed route is **NOT** to start DeepSeek directly, is **NOT** to start blind walk-forward directly, and is **NOT** to enter Phase 12. Safety flags unchanged across this docs-only closeout. Phase 12 remains **FORBIDDEN**. |
...
DAILY_BUCKET_ONLY` (explicitly **NOT** full data coverage
accepted, explicitly **NOT** intraday-kline accepted).

**Closeout PR type:** docs-only.

### What this closeout records

  - **PR #71 implemented the price path adapter** at
    `app/adaptive/post_discovery_price_path_adapter.py`.
  - **The D-B / B1.1 evidence runner can load
    `data/historical_market_store/` daily top mover
    summary** and resolve per-record price paths from it.
  - **Price path resolution is now record-level**
    (`list[PricePathResolution | None]` aligned by index
    with the input record list), not symbol-only and no
    longer first-record-wins.
  - **Operator-supplied price paths cannot turn a future
    price into the discovery anchor.**
    `build_operator_supplied_price_path_resolution()`
    rejects any candidate point with
    `timestamp > first_seen_time_utc_ms` for
    `first_seen_price`.
  - **The Lookahead Guard is hard-enforced at record
    level**: `first_seen_time` only from a D-A audited
    record / `EventRepository` event timestamp;
    `first_seen_price` only from the latest price point
    at-or-before `first_seen_time` (operator anchor /
    capture-path anchor / containing-day open); price
    path after `first_seen_time` only used for post-window
    outcome audit; peak / trough / MFE / MAE only computed
    after the window closes and never feed live radar
    score, candidate promotion, the Risk Engine, the
    Execution FSM, `symbol_limit`, anomaly threshold,
    candidate-pool capacity, or Regime weights.
  - **The v0 path is daily-bucket** (`kline_interval_used
    = 1d`); it is **not** a 1m / 5m intraday K-line path.
  - **B1.1 toolchain is accepted.**
  - **The PR #71 evidence runner evaluated 300 records**
    and emitted **300 `POST_DISCOVERY_OUTCOME_EVALUATED`
    events** plus **1
    `POST_DISCOVERY_OUTCOME_REPORT_GENERATED` event**.
  - **The price path adapter works at daily-bucket
    resolution** (`kline_interval_used = 1d`).
  - **The current local Historical Market Store only
    provided price paths for 17 / 300 records.**
  - **283 / 300 records remain missing price path.**
  - **`RAVEUSDT` and `STOUSDT` remain unresolved** because
    no top mover row covers their `first_seen_time`.

### B1.1 main evidence run output
...
Notable symbol price path summary:
  ...
  - **`RAVEUSDT`**: `loaded = false`,
    ...
    `record_count = 17`, `source = absent`.
  - **`STOUSDT`**: `loaded = false`,
    `loaded_record_count = 0`,
    `missing_reason = no_top_mover_row_covering_first_seen_time`,
    `record_count = 3`, `source = absent`.
...
### What this acceptance level MEANS
  ...
  - **The B1.1 toolchain is accepted.** PR #71 ships a
    working price path adapter, a working record-level
    resolver, an operator-supplied path Lookahead Guard,
    and a working evidence runner. The runner consumes
    the real D-A export, evaluates 300 records, emits
    300 `POST_DISCOVERY_OUTCOME_EVALUATED` events, and
    emits one `POST_DISCOVERY_OUTCOME_REPORT_GENERATED`
    aggregating them. The diagnostics columns
    ...
    designed. The marker
    `B1_1_PRICE_PATH_MAIN_EVIDENCE_CHECK = PASS` confirms
    the toolchain half of the acceptance.
  - **Price path resolution works at daily-bucket
    resolution.** `kline_interval_used = 1d`. For the
    containing day only the close at `day_end_ms` is
    emitted; for subsequent days the high / low are
    stamped at `day_end_ms` (intra-day timestamps are
    unknown). This is the **daily-bucket only** half of
    the acceptance.
  - **The current local Historical Market Store only
    provided price paths for 17 / 300 records.** 283 /
    300 records remain `absent`, broken down into
    `no_first_seen_time = 110`,
    `no_top_mover_row_covering_first_seen_time = 133`,
    and `insufficient_post_first_seen_points = 40`. This
    is the **partial data coverage** half of the
    acceptance.
  - **`RAVEUSDT` and `STOUSDT` remain unresolved.**
    Neither symbol has any top mover row in the local
    Historical Market Store covering its
    `first_seen_time`. They cannot be triaged as severe
    missed tails by B1.1 alone yet; they require
    intraday price-path completeness or explicit
    data-gap triage before the Severe Missed Tail Triage
    slice can consume them.

### What this acceptance level does NOT MEAN

  - **B1.1 does NOT solve intraday price path
    completeness.** The adapter is daily-bucket only;
    283 / 300 records remain missing under the local
    Historical Market Store; a 1m / 5m intraday adapter
    + intraday store are NOT shipped by PR #71.
  - **B1.1 does NOT solve direction.** No `long` /
    `short` / `entry` / `exit` / `stop` / `target` /
    `position_size` / `leverage` field is emitted; the
    labels and the price path are descriptive only.
  ...
  - **B1.1 does NOT authorise auto-tuning.** The
    price-path resolutions and outcome labels MUST NOT
    drive `symbol_limit` expansion, anomaly threshold
    changes, candidate-pool capacity changes, Regime
    weight changes, or any other runtime knob. "Looking
    at the answer key" against the post-hoc D-A
    reference set or the daily-bucket price path is
    forbidden.
  - **B1.1 does NOT authorise DeepSeek trade decisions.**
    DeepSeek remains read-only / sandbox-only / offline
    under the AI Layer Constitution; B1.1's price paths
    and outcome labels are **not** trade-authorisation
    surface for DeepSeek or any other LLM.
...
### Next allowed route (paper-only; gated, sequential)
  ...
  - **Historical Kline Store Builder / Intraday Price
    ...
    approval. It is **not** a precondition for B2 and is
    **not** the next slice. This PR does **not** open it.
  - **B1.1 does not solve intraday price path
    completeness.** This closeout does not claim
    otherwise.
  - **B1.1 does not solve direction.** This closeout
    does not claim otherwise.
  - **B1.1 does not prove strategy profitability.** This
    closeout does not claim otherwise.
  - **B1.1 does not authorise auto-tuning.** This
    closeout does not claim otherwise.
  - The next allowed route is **NOT** to start DeepSeek
