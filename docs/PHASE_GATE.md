...
| #          | Title                                                                | State                       | Detail                                                                 |
| ---------- | -------------------------------------------------------------------- | --------------------------- | ---------------------------------------------------------------------- |
| 11C-OFFLINE-RULE-SANDBOX-REPLAY | Offline Rule Sandbox Replay v0 (*离线规则沙箱回放 v0*; paper / report / sandbox-only deterministic offline counterfactual layer that lets an operator project the likely impact of a hypothetical rule change on **discovery quality** over a frozen historical reference window; **NOT** auto-tuning, **NOT** runtime-config writeback, **NOT** trade authority, **NOT** the AI Layer's involvement in the Risk Engine / Execution FSM / Strategy Engine / Runtime Config, **NOT** Paper Shadow, **NOT** Blind Walk-forward, **NOT** Phase 12) | **IN_REVIEW (after this implementation PR; not `ACCEPTED` until maintainer review)** | Phase 11C Offline Rule Sandbox Replay v0 is **`IN_REVIEW`** after the implementation PR. The PR ships `app/sandbox/__init__.py` + `app/sandbox/offline_rule_sandbox.py` (the engine + dataclasses: `OfflineRuleSandboxScenario`, `HypotheticalRuleChange`, `OfflineRuleSandboxInput`, `OfflineRuleSandboxResult`, `OfflineRuleSandboxReport`, `OfflineRuleSandboxEngine`; closed `change_type` vocabulary `loosen` / `tighten` / `no_change`; closed deterministic impact table mapping each rule_name to a per-metric direction; closed `recommendation_level` vocabulary `REVIEW_ONLY` / `INCONCLUSIVE` / `PROMISING_FOR_PAPER_SHADOW` / `RISKY` / `REJECTED_BY_EVIDENCE`; recursive `_assert_no_forbidden_keys` guard refusing every direction / sizing / leverage / stop / target / risk-budget / order / runtime-config-patch / "signal-to-trade" / "should buy/short" / "apply" / "deploy" / "enable_live" alias; hard pins `phase_12_forbidden=True`, `auto_tuning_allowed=False`, `writes_runtime_config=False`, `trade_authority=False`, `sandbox_only=True`, `ai_output_can_be_truth=False`, `ai_output_can_be_training_label=False`, `ai_output_can_be_tail_label=False`, `ai_output_can_be_strategy_sample=False` re-pinned at every `to_dict()` boundary), the runner `scripts/run_offline_rule_sandbox_replay.py` (paper / report / sandbox-only; reads only local files under `--block-b-report` / `--block-c-report` / `--ai-checkpoint` / `--scenario-file` and the optional baseline / outcome / reject / severe-miss / replay / reflection summary paths; writes only files under `--output-dir`; emits one descriptive `offline_rule_sandbox_report.json` plus matching `.md` summary; falls back to a deterministic `source=example_fixture` scenario when `--scenario-file` is missing; never imports `app.risk` / `app.execution` / `app.exchanges` / `app.telegram` / `app.config`; never imports any HTTP / network library), the `tests/unit/test_offline_rule_sandbox_replay.py` unit-test module covering every brief-mandated check (builds scenario without writing runtime config; hypothetical rule change is **not** a runtime patch; computes delta metrics deterministically; missing evidence → `INSUFFICIENT_EVIDENCE` / `INCONCLUSIVE`; data gap warnings preserved; `recommendation_level` never `APPLY` / `DEPLOY` / `TRADE`; `auto_tuning_allowed=False` / `writes_runtime_config=False` / `trade_authority=False` / `phase_12_forbidden=True` on every payload; forbidden fields absent at every nesting depth; runner does not import `app.risk` / `app.execution` / `app.exchanges` / `app.telegram` / `app.config`; no DeepSeek / LLM / network call path with a `socket.socket`-monkeypatched belt-and-braces test; JSON output serialisable; deterministic output across two runs), and the new phase doc `docs/PHASE_11C_OFFLINE_RULE_SANDBOX_REPLAY.md`. **Status taxonomy is intentionally NOT `ACCEPTED`.** Per-scenario status is `EVALUATED` / `INCONCLUSIVE` / `INSUFFICIENT_EVIDENCE` (no per-scenario `ACCEPTED` exists). Tests on this PR: `tests/unit/test_offline_rule_sandbox_replay.py` 30/30 PASS; full `tests/unit` 3357/3357 PASS. The runner connects to no network, calls no LLM / DeepSeek, opens no Telegram socket, signs no request, never modifies `app.config`, and never reads a private API. **NOT live trading. NOT trade authority. NOT auto-tuning. NOT runtime-config writeback. NOT the DeepSeek hot path. NOT Telegram live outbound. NOT a per-phase `ACCEPTED` gate. NOT a strategy profitability proof. NOT a direction call. NOT Paper Shadow. NOT Blind Walk-forward. NOT Phase 12.** A scenario with `recommendation_level=PROMISING_FOR_PAPER_SHADOW` only authorises Paper Shadow Strategy Validation preparation work (paper / read-only); it does **NOT** open Phase 12. Every emitted payload re-pins `auto_tuning_allowed=False` and `writes_runtime_config=False`; the constants are hard-pinned at every `to_dict()` boundary. The Risk Engine remains the single trade-decision gate. Phase 12 remains **FORBIDDEN**. |
