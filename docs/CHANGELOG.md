...
**Added — `app/live/` package:**
  ...
  - `api_config.py` — env-driven `BinanceApiConfig` / `TelegramApiConfig`
    / `DeepSeekApiConfig` / `GeneralLiveConfig` / `LiveApiConfig.from_env`.
    Reuses PR110's `LiveRuntimeMode` (`app.core.enums`, default
    `LIVE_SHADOW`); PR111 never escalates to LIVE_LIMITED.
  - `binance_income.py` — maps Binance `/fapi/v1/income` rows into
    PR110's Capital Event contract (`app.live.capital_event`:
    `LiveCapitalEvent` / `CapitalEventType` / `CapitalEventLedger`).
    Funding never mixed with trading PnL;
    `net_strategy_pnl = realized - fees + funding` (computed by PR110's
    ledger). Funding without a trade_id is marked
    `UNATTRIBUTED_PENDING_POSITION_LINK`; unknown income types are
    preserved verbatim (never dropped, never mis-mapped).
...
`CapitalProfileId` (default `L0_SHADOW`). **HANDOFF(PR113/PR114):**
position-level funding attribution must be added before real live PnL is
final.

### PR110 — Live Foundation v0 — Live Path Isolation + LIVE_SHADOW/LIVE_LIMITED Mode Guard + Capital Profile Ladder + Capital Event Contract + Right-tail Leverage Gate + Telegram Operator Contract: IN_REVIEW
...
**Runtime effect:** **none on real trading.** PR110 does NOT enable live
trading, does NOT connect the Binance private API, does NOT place /
cancel a real order, does NOT change leverage / margin mode, does NOT
enable real Telegram outbound, does NOT enable Phase 12, and does NOT
let AI / Telegram / blind / replay results influence any live decision.

**Why now.** The roadmap switched from large-scale cloud blind testing
(uneconomic data ingestion on the current server; Kiro subscription
ending) to a 10U small-capital live-preparation posture. PR110 builds
the hard pre-live boundaries so the existing blind/replay/sim stack can
never leak into a real order.

**What changed (new, additive, isolated — `app/live/`).**

  - **Live Path Isolation** (`app/live/path_isolation.py`,
    `app/live/gateway.py`). Every order intent carries an
    `OrderSource` (`SIM` / `BLIND` / `REPLAY` / `PAPER_SHADOW` /
    `LIVE`). `LivePathIsolationGuard` admits ONLY `OrderSource.LIVE`;
    every other source is refused with a `LIVE_PATH_BLOCKED` event and
    a `LivePathIsolationViolation`. `classify_source_module` fail-safe
    maps MockExchange / BlindWalkForwardRunner / ReplayFeedProvider /
    PaperShadowStrategyBridge / SimulatedCapitalFlow to a blocked
    source (never `LIVE`). `LiveExecutionGateway.submit_order` is the
    single reserved live entry point and refuses all submissions in
    PR110 (no execution adapter exists).
  - **Runtime Mode Guard** (`app/live/runtime_mode.py`). Two modes:
    `LIVE_SHADOW` (空盘跑; read-only, no real orders ever) and
    `LIVE_LIMITED` (有资金跑; real small-capital trading in principle,
    not implemented in PR110). Default mode is `LIVE_SHADOW`;
    `LiveModeGuard.from_config` ALWAYS boots in `LIVE_SHADOW` so a
    restart / default config can never silently enter `LIVE_LIMITED`.
    Arming `LIVE_LIMITED` requires the `/mode live_limited` →
    `/confirm_live CODE` handshake, a profile that allows real orders,
    and an armed kill switch. Emits `LIVE_MODE_SWITCH_REQUESTED` /
    `_CONFIRMED` / `_REJECTED`, `LIVE_LIMITED_ARMED` / `_DISARMED`,
    `LIVE_SHADOW_ACTIVE` / `LIVE_LIMITED_ACTIVE`.
  - **Capital Profile Ladder** (`app/live/capital_profile.py`).
    `L0_SHADOW` .. `L8_10M_CAPITAL_PRESERVATION` (1U → 10,000,000U).
    `L0_SHADOW.real_orders_allowed=False`; `L1_10U_PROBE`
    `max_account_capital_usdt=10`. Small profiles may be aggressive;
    large profiles enforce tighter liquidity floor / symbol exposure /
    slippage. Escalation is NEVER automatic (`AUTO_ESCALATION_ALLOWED=
    False`); `detect_profile_mismatch` flags when adjusted equity
    leaves the active band and demands operator action.
  - **Capital Event Contract** (`app/live/capital_event.py`).
    `CapitalEventType` (EXTERNAL_DEPOSIT / EXTERNAL_WITHDRAWAL /
    REALIZED_PNL / REALIZED_LOSS / FEE / FUNDING_FEE / FUNDING_INCOME /
    MANUAL_ADJUSTMENT / TRANSFER_IN / TRANSFER_OUT / PROFIT_HARVEST /
    CAPITAL_REBASE). `CapitalEventLedger` keeps trading PnL strictly
    separate from external flows and fees/funding — a deposit is never
    counted as profit and a withdrawal is never counted as a loss.
  - **Right-tail Leverage Gate** (`app/live/leverage_gate.py`).
    `evaluate_right_tail_leverage_permission` is deterministic; inputs
    are deterministic evidence + capital profile + runtime mode + risk
    state only. Refuses AI / LLM / Telegram / blind / future-label
    input (`AI_INPUT_FORBIDDEN`), missing exit / stop plan, missing
    liquidity evidence, risk-off regime, systemic risk, drawdown
    warning, risk halt, profile-disallows-boost, no-floating-profit
    boost requests, slippage/spread over threshold, symbol exposure
    over threshold, and leverage over the profile max. Grants limited
    leverage only on strong deterministic right-tail structure when the
    profile allows it.
  - **Telegram Operator Contract**
    (`app/live/telegram_operator_contract.py`). Command contract
    (`/mode`, `/confirm_live`, `/capital_profile`, `/risk`,
    `/positions`, `/pnl`, `/pause`, `/resume`, `/kill_all`), card-type
    taxonomy, and the card field schema. Shadow cards show the planned
    entry/stop/take-profit but `real_order=False`, `order_id="--"`;
    live cards carry the real entry/exit/pnl/balance schema, but
    `real_order` is forced False in PR110
    (`LIVE_EXECUTION_ADAPTER_AVAILABLE=False`).

  - **Core additions.** `OrderSource` + `LiveRuntimeMode` enums
    (`app/core/enums.py`); 12 PR110 audit event types
    (`app/core/events.py`); `LivePathIsolationViolation` /
    `LiveModeViolation` / `LeverageGateViolation`
    (`app/core/errors.py`).
  - **Config.** New locked `live:` section (`app/config/schema.py`
    `LiveConfig`, `app/config/defaults.yaml`, `app/config/settings.py`
    properties). The schema refuses to load `live_trading`,
    `exchange_live_orders`, `binance_private_api_enabled`,
    `ai_trade_authority`, `trade_authority`, or
    `right_tail_live_boost_enabled` as True, and refuses
    `phase_12_forbidden=False`.
  - **Tests.** `tests/unit/test_pr110_live_foundation.py` (all 24
    brief-mandated cases).

**Safety flags (unchanged / asserted):** `phase_12_forbidden=true`,
...
`right_tail_live_boost_enabled=false`. The five Phase 1 safety flags
remain locked. **Phase 12 remains FORBIDDEN.** See
`docs/AMA_RT_LIVE_FOUNDATION_SPEC.md` and
...
**Type:** Implementation PR (paper / report / evidence-only
infrastructure). **Runtime effect:** **none on real trading; no
real Telegram outbound; no production / live Telegram channel;
no Telegram command authority; no Binance private API; no
auto-tuning; no AI hot path inside the trade-decision chain.**
Three new modules
...
helper), and one new phase doc
`docs/PHASE_11C_1D_D_G_BLIND_WALK_FORWARD_RUNNER.md`. No file
...
sources (`app/sim/simulation_clock.py`,
...
hot path. No database schema / migration. The new modules have
NO network, NO Telegram Bot API call, NO Binance private API
call, NO DeepSeek / LLM call, NO real exchange order, NO signed
endpoint, NO private websocket, NO API key, NO API secret, NO
real account id, NO authority over the Risk Engine, the
Execution FSM, or the Capital Flow Engine.

**What this PR ships.** The **seventh** anti-future-lookahead
infrastructure block of the strict blind walk-forward stack
defined by Phase 11C.1D-D (PR93, the *Strict Blind Walk-forward
Sim-Live Constitution*). PR100 strings PR94..PR99 substrate (the
...
forward-only historical sim-live blind runner. The runner
implements the `train` / `freeze` / `blind` / `score` /
`experience-update` loop that the Constitution requires, drives a
`SimulationClock` from `window.blind_start` to `window.blind_end`,
pulls one batch per step from the `ReplayFeedProvider`, marks the
`SimulatedCapitalFlowEngine` against the closed 1m candles in
each batch, forwards orders returned by an optional decision
callback to the `MockExchange`, forwards every fill back into the
`SimulatedCapitalFlowEngine` so the trade ledger and the equity
time-series stay consistent, writes a paper-only Telegram
sandbox transcript via the `TelegramSandboxOutbox`, records every
`NoLookaheadViolation` and every `BlindRunInvalidationReason`,
and scores **only** after `window.blind_end` (never inside the
blind window).
...
**Public surface added.**
  ...
  - `BlindWalkForwardWindow` — frozen description of one strict
    ...
    `blind_start`, `blind_end`, `score_time`, `reference_window`,
    `window_id`).
  - `BlindRunManifest` — frozen run manifest with deterministic
    `sha256:<hex>` hashes for `config_hash`, `rule_hash`,
    `feature_schema_hash`, `data_manifest_hash`,
    `universe_manifest_hash`, `fee_model_hash`,
    `slippage_model_hash`, `latency_model_hash`,
    `outage_model_hash`, `fill_model_hash`, plus
    `simulation_clock_start` / `_end`, `base_clock_step`
    (default `1m`), `allowed_timeframes`
    (`("1m", "5m", "15m", "1h", "4h", "1d")` by default),
    `ai_enabled_state` (`OFFLINE_ASOF_ONLY` /
    `OFFLINE_POST_WINDOW_ONLY`), `telegram_sandbox_state`
    (`SANDBOX_FILE_ONLY` / `DISABLED`), and
    `intrabar_ambiguity_policy` (`WORST_CASE` /
    `AMBIGUOUS_INTRABAR_PATH` / `AMBIGUOUS`). The manifest
    hard-pins `phase_12_forbidden=True`, `live_trading=False`,
  ...
  - `BlindRunInvalidationReason` — closed taxonomy mirroring
    PR100 brief §9 (`FUTURE_RECORD_ACCESS`, `CONFIG_DRIFT`,
  ...
  - `BlindRunScore` + `score_blind_run` — pure / deterministic /
    post-window scoring helper. NEVER consumed by the Risk
    Engine, the Execution FSM, the Capital Flow Engine, the
    Strategy Validator, the auto-tuner (which is forbidden in
    this phase), or any AI hot path.
  - `BlindWalkForwardRunnerConfig` — frozen runner configuration
    ...
    `strict_feature_asof=True`. Includes frozen artefact source
    bundles (`config_artefact`, `rule_artefact`,
    `feature_schema_artefact`, `data_manifest_artefact`,
    `universe_manifest_artefact`, `fee_model_artefact`,
    `slippage_model_artefact`, `latency_model_artefact`,
    `outage_model_artefact`, `fill_model_artefact`).
  ...
  - `AsOfFeatureCache` — feature cache keyed by `as_of_time`
    that NEVER discloses a future feature even if asked for one
    (returns `None` and bumps a future-access counter).
  - `BlindWalkForwardRunner` — the orchestrator itself
    ...
    `build_post_window_ai_summary`). Outputs every required
    ...
    `no_lookahead_violations.json`. Every payload is checked
    against the project-wide `FORBIDDEN_OUTPUT_FIELDS` guard;
    every payload re-pins the safety boundary.
...
auto-tuning inside blind window `False`, auto-tuning allowed
`False`, `phase_12_forbidden=True`. **Phase 12 remains
FORBIDDEN.**

**Acceptance authority.** A successful PR100 only authorises a
paper-only **blind-run checkpoint / operator evidence run**. It
does NOT authorise live trading, auto-tuning, real Telegram
outbound, real production / live Telegram channel, Telegram
command authority, the Binance private API, small-cap live
trading, or Phase 12.

**Tests.** New file
`tests/unit/test_blind_walk_forward_runner.py` ships **31
PASSING tests** covering all 30 brief-mandated scenarios plus
one bonus `INSUFFICIENT_EVIDENCE`-on-empty-run check on the pure
scoring helper. The full unit suite remains green
(`python -m pytest tests/unit -q` — 3584 PASSING).

### Phase 11C.1D-D-F — Telegram Sandbox Outbox v0 (PR99): IN_REVIEW

**Type:** Implementation PR (paper / report / evidence-only
infrastructure). **Runtime effect:** **none on real trading; no
real Telegram outbound; no production / live Telegram channel; no
...
`app/confirmation/`, `app/capital/`, `app/core/`, `app/main.py`
is touched. The existing PR94 / PR95 / PR96 / PR97 / PR98 sources
(`app/sim/simulation_clock.py`, `app/sim/time_wall_guard.py`,
`app/sim/historical_market_store.py`,
`app/sim/replay_feed_provider.py`, `app/sim/mock_exchange.py`,
`app/sim/pessimistic_fill_model.py`,
`app/sim/simulated_capital_flow.py`, `app/sim/trade_ledger.py`)
are reused **verbatim** and are NOT modified by this PR. The
existing Phase 10D Telegram fake / refusal boundary in
`app/telegram/` is **NOT** reused — PR99 has its own separate
sandbox path that NEVER touches the real outbound transport. No
event type wired into the runtime hot path. No database schema /
migration. The new module has NO network, NO Telegram Bot API
call, NO Telegram production token read, NO production / live
Telegram channel target, NO inbound Telegram command, NO Telegram
command authority, NO real exchange API, NO signed endpoint, NO
private websocket, NO API key, NO API secret, NO real account id,
NO authority over the Risk Engine, the Execution FSM, or the
Capital Flow Engine.

**What this PR ships.** The **sixth** anti-future-lookahead
infrastructure block of the strict blind walk-forward stack
defined by Phase 11C.1D-D (PR93, the *Strict Blind Walk-forward
Sim-Live Constitution*). PR93 §19's engineering route explicitly
authorises ONLY this PR (PR99) to begin the next paper-only step
after PR98. PR99 implements the deterministic, paper-only,
file-only Telegram notification surface that produces the
JSONL + Markdown transcripts blind-run reviewers will consume.
PR100 *Blind Walk-forward Runner v0* remains **FORBIDDEN by this
phase alone**.

**Public surface added.**

  - `TelegramSandboxMessageType` — closed taxonomy of exactly 13
    message types: `SIMULATED_ENTRY_ALERT`,
    `SIMULATED_EXIT_ALERT`, `RISK_REJECTION`, `FORCED_EXIT`,
    `STALE_FEED`, `OUTAGE`, `DATA_GAP`, `RIGHT_TAIL_CAPTURED`,
    `SEVERE_MISSED_TAIL`, `EQUITY_SUMMARY`,
    `FAILURE_LEDGER_SUMMARY`, `MONTHLY_BLIND_TEST_SUMMARY`,
    `AI_OPERATOR_BRIEFING_READY`. Each value is a paper-only
    descriptor; values appear only as JSON / Markdown values,
    never as field names, so the project-wide
    `app.sim.time_wall_guard.FORBIDDEN_OUTPUT_FIELDS` guard
    remains untouched.
  - `TelegramSandboxSeverity` — closed taxonomy `INFO` /
    `NOTICE` / `WARNING` / `CRITICAL`. Descriptive only; NEVER a
    runtime config patch.
  - Four mandatory simulated / no-live transcript labels
    (`SIMULATED_HISTORICAL_BLIND_TEST_LABEL` /
    `NO_LIVE_ORDER_LABEL` / `NO_REAL_CAPITAL_LABEL` /
    `NO_TELEGRAM_COMMAND_AUTHORITY_LABEL`, also exposed as the
    tuple `MANDATORY_LABELS`). Every rendered Markdown message
    carries all four labels, verbatim, on dedicated lines at the
    very top of the rendering, plus a re-pinned footer.
  - `TelegramSandboxMessage` — frozen, JSON-serialisable
    paper-only message (`message_id`, `timestamp_simulated`,
    `message_type`, `title`, `body`, `severity`, optional
    `symbol`, `evidence_refs`) with hard-pinned safety markers
    ...
    attempt to flip the safety flags. Construction refuses naive
    timestamps, unknown message types, and unknown severities.
  - `TelegramSandboxOutboxConfig` — frozen output configuration
    (`output_jsonl_path`, default
    `data/reports/telegram_sandbox_outbox.jsonl`;
    `output_markdown_path`, default
    `data/reports/telegram_sandbox_messages.md`; `append_mode`,
    default `False`; `include_evidence_refs`, default `True`;
    optional `max_message_body_chars`) with hard-pinned
    `sandbox_only=True`, `telegram_outbound_enabled=False`,
    `telegram_live_command_authority=False`,
    `telegram_production_channel_enabled=False`,
    `command_authority=False`. Construction refuses any attempt
    to flip the safety flags.
  - `TelegramSandboxOutbox` — paper-only / file-only Telegram
    sandbox outbox. Public methods `append_message(message)`,
    `append_messages(messages)`, `render_message(message)`,
    `write_jsonl(path=None)`, `write_markdown_transcript(path=None)`,
    `list_messages()`, `reset()`, `safety_payload()`, `to_dict()`.
    Public properties `config`, `sandbox_only` (=`True`),
    `telegram_outbound_enabled` (=`False`),
    `telegram_live_command_authority` (=`False`),
    `telegram_production_channel_enabled` (=`False`),
    ...
    `auto_tuning_allowed` (=`False`), `message_count`. The outbox
    exposes **no** public method with a Telegram / network verb
    (`send_message`, `send_document`, `send_telegram`, `post`,
    `get`, `connect`, `open_websocket`, `place_order`, `sign`,
    etc.).

**Strict blind walk-forward semantics.** Every payload is
deterministic (JSONL emitted with `sort_keys=True`). Every
`TelegramSandboxMessage` carries an explicit
`timestamp_simulated` (a timezone-aware UTC `datetime` enforced
through PR94's `ensure_utc_aware`). Every `to_dict()` payload
runs through both `assert_no_forbidden_fields` (project-wide PR94
guard) and a local outbox-specific guard that rejects every
Telegram-bot-token / production-channel-id / api-key / api-secret
/ real-exchange-order-id / signed-endpoint field name. Two
outboxes fed identical config + messages produce byte-identical
JSONL + Markdown sequences (verified by
`test_deterministic_output_across_two_independent_runs`).

**Hard safety boundary (Phase 11C.1D-D-F / PR99).**
`mode=paper`, `sandbox_only=True`, `simulated_only=True`,
`no_live_order=True`, `no_live_order_assertion=True`,
`no_real_capital_assertion=True`,
...
**What this PR does NOT do, and CANNOT do, regardless of any
caller's wishes:**
  ...
  - emit a real exchange order id, an api key, an api secret, a
    Telegram bot token, a production / live channel id, or a
...
Successful PR99 only authorises **PR100 — Blind Walk-forward
...
**Files shipped:** `app/sim/telegram_sandbox_outbox.py` (the core
implementation), `app/sim/__init__.py` (re-exports the new public
surface alongside the PR94 + PR95 + PR96 + PR97 + PR98 substrate),
`tests/unit/test_telegram_sandbox_outbox.py` (23 PASSING tests
covering all 21 brief-mandated scenarios plus 2 defensive extras:
closed-taxonomy + phase-name-string presence; `reset()` clears
the in-memory list but does NOT truncate any on-disk artefact),
`docs/PHASE_11C_1D_D_F_TELEGRAM_SANDBOX_OUTBOX.md` (the design /
acceptance brief), `docs/PROJECT_STATUS.md` (current-phase entry
prepended; prior 11C.1D-D-E entry preserved as historical
context), `docs/PHASE_GATE.md` (PR99 added to *Open / Reserved
phases*; only PR100 unlocked; Phase 12 row unchanged), and this
